...
export default function ProductForm() {
  ...
  const loadProduct = useCallback(async () => {
    ...
    try {
      ...
      setForm({
        ...
        subcategoriesText: Array.isArray(p.subcategoriesText) ? p.subcategoriesText.join("\n") : "",
  ...
  useEffect(() => {
    // After categories load / change, refresh subcategory options
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const url = isNew ? `${API_BASE}/api/products/add` : `${API_BASE}/api/products/${id}`;
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto p-8 text-slate-500 font-medium">Loading product…</div>
    );
  }
    ...
    <div className="admin-page admin-page--wide max-w-6xl mx-auto p-8">
      <header className="flex justify-between items-start mb-10">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">{isNew ? "New product" : "Edit product"}</h1>
          <p className="text-sm text-slate-400 font-medium mt-1">
            Structured data for nursery / plant commerce — matches your storefront and SEO.
          </p>
        </div>
        <button
          type="button"
      ...
      {error && (
        <div className="mb-6 rounded-2xl border border-red-100 bg-red-50 text-red-700 text-sm font-semibold px-4 py-3">{error}</div>
      )}
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-6 py-2.5 rounded-full text-xs font-black transition-all ${
              tab === t.id ? "bg-emerald-950 text-white shadow-lg" : "bg-slate-100/80 text-slate-500 hover:bg-slate-200"
            }`}
          >
            {t.label}
          </button>
      ...
      <form className="space-y-6" onSubmit={handleSave}>
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden p-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button type="button" onClick={slugFromName} className="px-4 py-3 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-tight hover:bg-slate-50">
                      From name
                    </button>
                  </div>
                </div>
                ...
                <label className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      set("categoryId", next);
                      // Immediately clear to avoid stale selection while options load
                      set("subcategoriesText", "");
                    }}
                  >
                ...
                <label className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <div className="flex flex-col gap-2">
                    <select
                      className="admin-input-flat cursor-pointer"
                      multiple
                      disabled={!form.categoryId || loadingSubcats || subcatOptions.length === 0}
                      value={linesToArray(form.subcategoriesText)}
                      onChange={(e) => {
                        const selected = Array.from(e.target.selectedOptions).map((o) => o.value);
                        set("subcategoriesText", selected.join("\n"));
                      }}
                      size={Math.min(8, Math.max(3, subcatOptions.length || 3))}
                    >
                      {subcatOptions.map((s) => (
                        ...
                        {loadingSubcats
                          ...
                          : !form.categoryId
                            ...
                            : subcatOptions.length
                              ? `${linesToArray(form.subcategoriesText).length} selected`
                ...
                <label className="flex flex-col gap-2 md:col-span-2 border-t pt-6 mt-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Short description (2–3 lines)</span>
                ...
                <label className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <textarea className="admin-input-flat" rows={4} value={form.highlightsText} onChange={(e) => set("highlightsText", e.target.value)} placeholder="NASA clean-air study pick" />
                </label>
              </div>
            </div>
          )}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <label className="flex flex-col gap-2">
                  ...
                  <input className="admin-input-flat" type="number" value={form.mrp} onChange={(e) => set("mrp", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Selling price (₹)</span>
                  <input className="admin-input-flat" type="number" required value={form.price} onChange={(e) => set("price", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Discount</span>
                  <div className="flex gap-1">
                    <input className="admin-input-flat flex-1" type="number" value={form.discount} onChange={(e) => set("discount", e.target.value)} />
                    <select className="admin-input-flat w-28 shrink-0 text-[10px]" value={form.discountType} onChange={(e) => set("discountType", e.target.value)}>
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <input className="admin-input-flat" type="number" value={form.gstPercent} onChange={(e) => set("gstPercent", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Stock qty</span>
                  <input className="admin-input-flat" type="number" required value={form.stock} onChange={(e) => set("stock", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Sold by</span>
                  <input className="admin-input-flat" value={form.soldBy} onChange={(e) => set("soldBy", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Response rate %</span>
                  <input className="admin-input-flat" type="number" value={form.responseRate} onChange={(e) => set("responseRate", e.target.value)} />
                </label>
              </div>
              <div className="flex gap-8 pt-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="checkbox" className="w-5 h-5 accent-emerald-600 rounded" checked={form.reviewsEnabled} onChange={(e) => set("reviewsEnabled", e.target.checked)} />
                  <span className="text-sm font-bold text-slate-600">Reviews enabled</span>
                </label>
              </div>
            </div>
          )}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <label className="flex flex-col gap-2">
                  ...
                  <input className="admin-input-flat" value={form.plantType} onChange={(e) => set("plantType", e.target.value)} placeholder="Indoor / Succulent" />
                </label>
                <label className="flex flex-col gap-2">
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <input className="admin-input-flat italic font-medium" value={form.botanicalName} onChange={(e) => set("botanicalName", e.target.value)} />
                </label>
                <label className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Height Value & Unit</span>
                  <div className="flex gap-2">
                    <input className="admin-input-flat flex-1" type="number" value={form.heightValue} onChange={(e) => set("heightValue", e.target.value)} />
                    <select className="admin-input-flat w-28 shrink-0" value={form.heightUnit} onChange={(e) => set("heightUnit", e.target.value)}>
                      {HEIGHT_UNITS.map((u) => (
              ...
              <div className="pt-4">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="checkbox" className="w-5 h-5 accent-emerald-600 rounded" checked={form.airPurifying} onChange={(e) => set("airPurifying", e.target.checked)} />
                  <span className="text-sm font-bold text-slate-600">Air purifying pick</span>
                </label>
              </div>
            </div>
          )}
          ...
          {tab === "seo" && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <h2 className="text-xl font-black text-slate-800">SEO & merchandising title</h2>
              <div className="space-y-6">
                <label className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-500 ml-1">Merchandising Title</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-8">
                  <div className="space-y-6">
                    ...
                    <label className="flex flex-col gap-2">
                      <span className="text-xs font-bold text-slate-500 ml-1">Meta Keywords</span>
        ...
        <div className="flex justify-end gap-3 sticky bottom-8 pt-4">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="px-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition-all font-sans"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className="px-12 py-3.5 bg-emerald-700 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-emerald-100 hover:bg-emerald-800 transition-all active:scale-95 flex items-center gap-2"
          >
