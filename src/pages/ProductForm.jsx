...
export default function ProductForm() {
  ...
  const removeImage = (index) => {
    const existingImgs = linesToArray(form.imagesText);
    const existingCount = existingImgs.length;

    if (index < existingCount) {
      // Removing an existing image URL/filename
      const updatedExisting = existingImgs.filter((_, i) => i !== index);
      set("imagesText", updatedExisting.join("\n"));
    } else {
  ...
  return (
    <div className="admin-page admin-page--wide max-w-6xl mx-auto p-4 md:p-8">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-10">
        <div>
          <p className="text-[9px] md:text-[10px] uppercase font-black tracking-widest text-emerald-700 mb-1 md:mb-2">Catalogue</p>
      ...
      <div className="flex flex-wrap gap-2 mb-8" role="tablist">
        {TABS.map((t) => (
          <button
            ...
            type="button"
            onClick={() => setTab(t.id)}
            className={`px-4 md:px-6 py-2 md:py-2.5 rounded-full text-[10px] md:text-xs font-black transition-all ${
              tab === t.id
      ...
      <form className="space-y-6" onSubmit={handleSave}>
        <div className="bg-white rounded-2xl md:rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden p-6 md:p-10">
          {/* ==================== OVERVIEW TAB ==================== */}
          {tab === "overview" && (
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 sticky bottom-4 md:bottom-8 pt-4">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="w-full sm:w-auto px-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-400 hover:bg-slate-50"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className="w-full sm:w-auto px-12 py-3.5 bg-emerald-700 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg hover:bg-emerald-800 active:scale-95 transition-all"
          >
